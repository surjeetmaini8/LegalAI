...
class EnhancedBNSClassifier:
    ...
    def train_ensemble_model(self, df: pd.DataFrame) -> Dict:
        ...
        for section in np.unique(y):
            if "Section 103" in section or "Section 101" in section:
                ...
                "Section 115" in section
                or "Section 117" in section
                or "Section 124" in section
            ):
                ...
                "Civil" in section
                or "Consumer" in section
                or "Insurance" in section
                or "Labor" in section
                or "IP" in section
                or "Banking" in section
        ...
        for i, text in enumerate(X_test):
            ...
            rule_prediction = section_mapping.get(
                rule_result["prediction"], "Other Offenses"
            )
        ...
        try:
            cv_scores = cross_val_score(
                self.rf_classifier, X_train_tfidf, y_train, cv=cv_folds
            )
        except:
    ...
    def predict_with_confidence(self, case_description: str) -> Dict:
        ...
        if rule_confidence > 0.7:
            ...
            final_prediction = (
                ml_prediction if ml_confidence > 0 else rule_result["prediction"]
            )
