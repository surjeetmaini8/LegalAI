...
class EnhancedBNSClassifierV2:
    ...
    def load_cases_from_db(self):
        ...
        df["full_text"] = df.apply(
            lambda row: f"{row.get('title', '')} {row.get('description', '')} {row.get('case_type', '')} {row.get('filing_number', '')}",
            axis=1,
        )
    ...
    def train_models(self, df: pd.DataFrame):
        ...
        cv_scores_type = cross_val_score(
            self.rf_classifier, X_train_tfidf, y_type_train, cv=5
        )
        cv_scores_priority = cross_val_score(
            self.gb_classifier, X_train_tfidf, y_priority_train, cv=5
        )
    ...
    def predict(self, case_description: str) -> dict:
        ...
        return {
            ...
            "overall_confidence": (
                type_confidence + priority_confidence + bns_result["confidence"]
            )
            / 3,
        }
